#ifdef COPYLEFT
/*
 *  This file is part of starnose. It was originally part of BarrelEmerge.
 *
 *  starnose is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU General Public License as published by
 *  the Free Software Foundation, either version 3 of the License, or
 *  (at your option) any later version.
 *
 *  starnose is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU General Public License for more details.
 *
 *  You should have received a copy of the GNU General Public License
 *  along with starnose.  If not, see <https://www.gnu.org/licenses/>.
 */
#endif
...
int main (int argc, char **argv)
{
    ...
    morph::Visual plt (win_width, win_height, "James-Wilson barrel simulation");
    plt.zNear = 0.001;
    plt.zFar = 50;
    plt.fov = 45;
    ...
    if (plot_a) {
        ...
        for (unsigned int i = 0; i<RD.N; ++i) {
            ...
            unsigned int idx = plt.addVisualModel (new morph::HexGridVisual<FLT> (plt.shaderprog, plt.tshaderprog,
                                                                                  RD.hg,
                                                                                  spatOff,
                                                                                  &RD.a[i],
                                                                                  zscale,
    ...
    if (plot_n) {
        spatOff = { xzero, 0.0, 0.0 };
        ngrid = plt.addVisualModel (new morph::HexGridVisual<FLT> (plt.shaderprog, plt.tshaderprog,
                                                                   RD.hg,
                                                                   spatOff,
                                                                   &RD.n,
                                                                   zscale,
    ...
    if (plot_contours) {
        ...
        morph::HexGridVisual<FLT>* hgv_contours = new morph::HexGridVisual<FLT> (plt.shaderprog, plt.tshaderprog,
                                                                                 RD.hg,
                                                                                 spatOff,
                                                                                 &zeromap,
                                                                                 null_zscale,
    ...
    if (plot_a_contours) {
        spatOff = { xzero, 0.0, 0.0 };
        a_ctr_grid = plt.addVisualModel (new morph::HexGridVisual<FLT> (plt.shaderprog, plt.tshaderprog,
                                                                        RD.hg,
                                                                        spatOff,
                                                                        &zeromap,
                                                                        null_zscale,
    ...
    if (plot_dr && do_dirichlet_analysis == true) {
        spatOff = { xzero, 0.0, 0.0 };
        morph::HexGridVisual<FLT>* hgv_dr = new morph::HexGridVisual<FLT> (plt.shaderprog, plt.tshaderprog,
                                                                           RD.hg,
                                                                           spatOff,
                                                                           &zeromap,
                                                                           null_zscale,
    ...
    if (plot_guide) {
        ...
        for (unsigned int j = 0; j<RD.M; ++j) {
            plt.addVisualModel (new morph::HexGridVisual<FLT> (plt.shaderprog, plt.tshaderprog,
                                                               RD.hg,
                                                               spatOff,
                                                               &RD.rho[j],
                                                               null_zscale,
    ...
    if (plot_guidegrad) {
        ...
        for (unsigned int j = 0; j<RD.M; ++j) {
            ...
            gg_c = -(gg_m * ming);
            // The second is the colour scaling.
            morph::Scale<FLT> ggd_cscale; ggd_cscale.setParams (gg_m, gg_c);
    ...
    try {
        ...
        while (sighandling::finished == false) {
            ...
            if ((RD.stepCount % plotevery) == 0) {
                ...
                vector<FLT> ctrmap = morph::ShapeAnalysis<FLT>::get_contour_map (RD.hg, RD.c, RD.contour_threshold);
                ...
                if (plot_a_contours) {
                    vector<FLT> actrmap = morph::ShapeAnalysis<FLT>::get_contour_map (RD.hg, RD.a, RD.contour_threshold);
            ...
            if ((RD.stepCount % logevery) == 0) {
                ...
                vector<list<morph::Hex> > sv_ctrs = morph::ShapeAnalysis<FLT>::get_contours (RD.hg, RD.c, RD.contour_threshold);
    ...
    // Extract contours
    vector<list<morph::Hex> > ctrs = morph::ShapeAnalysis<FLT>::get_contours (RD.hg, RD.c, RD.contour_threshold);
    {
